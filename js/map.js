var mapVisualisation = function(data){
	...
	var infoText = function(){
		...
		var commandTable = {
		    info_button_map_toggle: function() { $('#info-text-map').toggle();},
		    info_text_map_close: function() { $('#info-text-map').hide();},
		    1: function(){ $("#info-button-map-toggle").css('background-position', '-28px -0.5px');},
		    0: function(){ $("#info-button-map-toggle").css('background-position', '0px -0.5px');} 
		};
		//keeps track if the info button is currently clicked
		var clicked = 0;
		//hover for the info button
	   	$('#info-button-map-toggle').hover( function(){
		...
		$('.info-button-map').on('click', function(){
